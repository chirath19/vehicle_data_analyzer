...
df1 = df.sort_values(by='Posted Date', ascending=False)
...
app.layout = html.Div([
    html.Div([
        html.Div([
            html.H1('Vehicle Data Analyzer', className='title_text ', style={'color': 'white', 'padding-top': '10px'})
...
def update_vehicle_latest(count, type_latest, status_latest):
    if type_latest != 'all' and status_latest != 'all':
        df1_filtered = df1.loc[(df1['Vehicle Type'] == type_latest) & (df1['Vehicle Status'] == status_latest)]
    elif type_latest != 'all':
        df1_filtered = df1.loc[df1['Vehicle Type'] == type_latest]
    elif status_latest != 'all':
        df1_filtered = df1.loc[df1['Vehicle Status'] == status_latest]
    else:
        df1_filtered = df1
